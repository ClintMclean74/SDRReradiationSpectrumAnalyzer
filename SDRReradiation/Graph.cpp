...
//uint32_t Graph::SetData(void* data, uint32_t length, uint8_t seriesIndex, SignalProcessingUtilities::DataType dataType, bool complex, double iOffset, double qOffset, bool swapIQ, bool insertAtEnd)
...
char* Graph::SetGraphFrequencyRangeText(const char *rangeText, FrequencyRange* frequencyRange, uint8_t textIndex, bool adjustForSelectedRegion)
{
	...
	if (visible)
	{
		...
		if (showLabels)
		{
			...
			for (int i = 0; i < textCount; i++)
			{
				...
				textStartHeight -= (labelHeight * Graphs::FONT_SCALE) * 5;
			}
			...
			if (showXLabels)
			{
				...
				for (int i = 0; i < labelsCount; i++)
				{
					...
					labelWidth *= Graphs::FONT_SCALE;

					GraphicsUtilities::DrawText(textBuffer, (i == 0 ? 0 : xPos - labelWidth / 2), -100, 0, Graphs::FONT_SCALE);
		...
		if (dataSeriesSameScale)
		{
			...
			if (showLabels)
			{
				...
				for (int i = 0; i < labelsCount; i++)
				{
					...
					labelHeight *= Graphs::FONT_SCALE;

					GraphicsUtilities::DrawText(textBuffer, -labelWidth, yPos, 0, Graphs::FONT_SCALE);
				...
				GraphicsUtilities::DrawText(textBuffer, -xPos, height/2 - labelWidth/2, 0, Graphs::FONT_SCALE, 90);
			}
		}
